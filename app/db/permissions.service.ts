...
export async function expireAllPendingPermissions(): Promise<void> {
  console.log('🔒 Expiring all pending permissions...');
  ...
  try {
    ...
    console.log(`Found ${pendingPermissions.length} pending permissions to expire`);
    
    const result = db.update(permissionRequests)
      .set({ 
    ...
    console.log(`✅ Expired ${result.changes} pending permission requests`);
  } catch (error) {
    console.error('❌ Failed to expire permissions:', error);
    throw error;
  }
}
